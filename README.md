...
| 파일 | 내용 |
|---|---|
| [`docs/scheduled-agents.md`](docs/scheduled-agents.md) | 파이프라인을 감시하는 클라우드 스케줄 작업 3종(운영 알림 메일·주간 리포트·아침 점검)의 명세와 프롬프트 전문 |

n8n 워크플로 자체의 사양은 저장소가 아니라 별도 위키(`wiki/spec/`)에 있다.
...
| 항목 | 상태 | 이유 |
|---|---|---|
| **국가 필터 · 유형 필터** | ✅ 구현 완료 | 임원보고서 4-1이 "국가별, 리스크 유형별 필터"를 명시한다 (2026-08-12 정정). 유형 필터도 구현됨(`TypeFilter`) |
...
| **위험 보고 패널 (2d)** | ✅ 구현 완료 | 제출하면 `risk_candidates`에 저장되고 관리자 승인을 거쳐 캘린더에 오른다. anon INSERT는 RLS로 "직접 입력·승인대기·미승격" 조건에서만 허용 |
| **경로 분석 탭 (2b)** | 🟡 국가 단위로 구현 (2026-09-05) | 좌표가 없어 노선 교차 판정은 여전히 불가 → Q-012. 지나는 **국가**의 이벤트를 business-rules §6으로 집계해 보여준다. 대안 경로·예상 지연시간은 근거가 없어 "준비중"으로 비워 뒀다 |
| **`status=cancelled` 별도 표시** | ✅ 구현 완료 | 본문에서는 빼고 날짜 상세 하단에 "취소" 절로 따로 보여준다 (`DayDetail`) → Q-024 #1 해소 |
...
| `status = cancelled` | 조회는 하되 본문 제외 | 날짜 상세에서 "취소" 절로 별도 표시한다 (WF-3 사양대로) |
